...
(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('three'), require('@egjs/component'), require('react')) :
  typeof define === 'function' && define.amd ? define(['three', '@egjs/component', 'react'], factory) :
  (global = global || self, global.View3D = factory(global.THREE, global.Component, global.React));
}(this, (function (THREE, Component, React) { 'use strict';
  ...
  ***************************************************************************** */
  ...
  const createKeyFrameTracks = (objectName, property, durationArray, vectorPositionArray) => {
    const keyFrame = new THREE.VectorKeyframeTrack(`${objectName}${property}`, durationArray, vectorPositionArray);
    return keyFrame;
  };
  /**
  ...
  const createIdleAnimation = view3DRef => {
    ...
    const vectorPositionArrayIdle = createVectorPositionArray(new THREE.Vector3().toArray(), new THREE.Vector3().toArray());
    ...
    const idleAnimation = new THREE.AnimationClip(`${view3DRef.animator.clips.length + 1}.Idle(def)`, -1, idleKeyFrameTracks);
  ...
  const RecordAnimation = () => {
    ...
    dragedMeshes.forEach((value, key) => {
      ...
      if (counter === dragedMeshes.size - 1) {
        ...
        const disassembleAnimation = new THREE.AnimationClip(`${view3DRef.animator.clips.length + 1}.disAssemblev2`, -1, disassembleKeyFrameTracks);
        ...
        const assembleAnimation = new THREE.AnimationClip(`${view3DRef.animator.clips.length + 1}.assemblev2`, -1, assembleKeyFrameTracks);
  ...
  class FixedRaycaster {
    ...
    _raycast(obj, raycaster, intersects) {
      ...
      _ray.copy(raycaster.ray).applyMatrix4(_inverseMatrix$1);
      let intersection;
  ...
  const listenAnnotationAdd = view3D => {
    ...
    canvas.addEventListener("dblclick", evt => {
      ...
      newAnnotation.uuid = THREE.MathUtils.generateUUID();
  ...
  const getBarycentricWeight = (p, vertices) => {
    ...
    const faceSize = new THREE.Vector3().crossVectors(new THREE.Vector3().subVectors(v1, v2), new THREE.Vector3().subVectors(v1, v3)).length();
    ...
    loadAnimation(animationIndex) {
      ...
      return Promise.all([Promise.all(pendingNodes), Promise.all(pendingInputAccessors), Promise.all(pendingOutputAccessors), Promise.all(pendingSamplers), Promise.all(pendingTargets)]).then(function (dependencies) {
        ...
        var _loop = function () {
          ...
          if (node === undefined) return 1; // continue
          node.updateMatrix();
  ...
  var modules = {
    ...
    rawGLTF: rawGLTF,
    ChangeEnvmaps: ChangeEnvmaps,
    StartRecordingAnimations: StartRecordingCustomAnimations,
    StopRecordingAnimations: StopRecordingCustomAnimations
  };
